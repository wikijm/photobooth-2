<?php
    $closeBtnTarget = "#";
    $closeBtnClasses = "!w-12 !h-12 relative flex items-center text-lg text-white justify-center !ml-auto p-4 cursor-pointer border border-white rounded-md transition-all hover:bg-white hover:text-black";
...
<div id="gallery" class="fixed top-0 left-0 z-50 w-full h-screen bg-black bg-opacity-80 hidden flex-col items-center text-white rotarygroup backdrop-blur-md [&.gallery--open]:flex">
        ...
        <?php
            $galleryCount = 0;
            if (empty($imagelist)) {
                $gallery = 'Leider sind keine Bilder vorhanden.';
            } else {
                $gallery = '';
                foreach ($imagelist as $image) {
                    ...
                    $filename_photo = $gallery_path . $config['foldersRoot']['images'] . DIRECTORY_SEPARATOR . $image;
                    ...
                    if (is_readable($filename_photo)) {
                        ...
                        $imageinfoThumb = getimagesize($gallery_path . $filename_thumb);
                        ...
                        if($config['gallery']['figcaption'] && $dateObject) {
                            $figcaption = '<figcaption class="text-white text-opacity-60 text-xs font-light pt-2"><i class="'. $config['icons']['date'] . '"></i> '. $dateObject->format($config['gallery']['date_format']).'</figcaption>';
                        }

                        $gallery .= '
                            <a href="'. $filename_photo.'" class="rotaryfocus" data-size="'.$size.'"data-med="'.$med.'">
                                <figure class="w-full h-0 pb-[67%] relative">
                                    <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">'. getLoader("") .'</div>
                                    <img class="w-full h-full absolute left-0 top-0 lazy object-cover z-20" data-src="'. $filename_thumb .'" alt="'. $image.'" >
                                </figure>
                                '.$figcaption.'
                            </a>';
                    }
            ...
            <div class="w-full max-w-7xl flex flex-col">
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3" id="galimages">
                    <?php 
                        ...
                        //     echo '<div class="gallery-close"><div class="w-full h-0 pb-[67%] relative">';
                        //     echo getBoothButton("newPhoto", $config['icons']['take_picture'], "newpic", "gallery");
                        //     echo '</div><div class="w-full h-3"></div></div>';
                        // }
                    ?>
                </div>
            </div>
